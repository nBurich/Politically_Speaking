...
class Personal extends Component {
  ...
  render() {
    ...
    if (member.twitter_account) {
      twitter = <a href={`https://twitter.com/${member.twitter_account}`} target="_blank" rel="noopener noreferrer">@{member.twitter_account}</a>
    } else if (member.twitter_id) {
      twitter = <a href={`https://twitter.com/${member.twitter_id}`} target="_blank"rel="noopener noreferrer" >@{member.twitter_id}</a>
    }
    if (member.district) {
      chamber = 'house'
    } else {
